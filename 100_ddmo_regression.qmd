...
Sales figures from a marketing campaign.
...
Is there an ideal $f(X)$? In particular, what is a good value for $f(X)$ at any selected value of $X$, say $X = 4$? There can be many $Y$ values at $X=4$.
A good value is
$$
f(4) = E(Y |X = 4).
$$
...
#### Local Regression (Smoothing)
 ...
 $$
where $\cal{N} (x)$ is some neighborhood of $x$.
...
If we add the two components (reducible and irreducible error), we get the MSE in @eq-biasvariance.
