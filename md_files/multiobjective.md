...
SpotOptim supports multi-objective optimization functions with automatic detection and flexible scalarization strategies.
The parameter `fun_mo2so` can be used to convert multi-objective values to single-objective values.
If the function `fun` returns several objective function values, say $f_1, f_2, \ldots, f_p$, these  values are stored in `y_mo` and can be be used by `fun_mo2so`.
The `fun_mo2so` parameter is optional. If it is `None`, which is the default, only the first objective $f_1$ is used as single-objective and the remaining objectives $f_2, \ldots, f_p$ are stored in `y_mo`, but not further used in the optimization process.
...
::: {#exm-default}
#### Default Behavior (Use First Objective)
...
@fig-zdt1-plot visualizes the Pareto front:

```{python}
#| label: fig-zdt1-plot
#| fig-cap: "ZDT1 Convex Pareto Front"
...
```{python}
#| label: print-zdt1-characteristics
...
@fig-zdt2-plot visualizes the Pareto front:

```{python}
#| label: fig-zdt2-plot
#| fig-cap: "ZDT2 Non-Convex Pareto Front"
...
@fig-zdt3-plot visualizes the Pareto front:

```{python}
#| label: fig-zdt3-plot
#| fig-cap: "ZDT3 Disconnected Pareto Front"
fig, ax = plt.subplots(figsize=(8, 6))
ax.scatter(y_non_optimal[:, 0], y_non_optimal[:, 1], 
          alpha=0.3, label='Non-optimal', s=20)
ax.plot(y_pareto[:, 0], y_pareto[:, 1], 
...
@fig-zdt4-plot visualizes the Pareto front:

```{python}
#| label: fig-zdt4-plot
#| fig-cap: "ZDT4 Multimodal Objective Space"
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
...
@fig-zdt6-plot visualizes the Pareto front:

```{python}
#| label: fig-zdt6-plot
#| fig-cap: "ZDT6 Non-Uniform Density"  
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
...
@fig-dtlz2-plot visualizes the Pareto front:

```{python}
#| label: fig-dtlz2-plot    
#| fig-cap: "DTLZ2 Spherical Pareto Front (3 objectives)"
...
@fig-schaffer-n1-plot visualizes the Pareto front:

```{python}
#| label: fig-schaffer-n1-plot
#| fig-cap: "Schaffer N1 Objective Space"
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
...
@fig-fonseca-fleming-plot visualizes the Pareto front:

```{python}
#| label: fig-fonseca-fleming-plot
#| fig-cap: "Fonseca-Fleming Concave Front"
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
...
```{python}
#| label: eval-zdt1-scalarization-diff
scalarizations = {
...
@fig-zdt1-scalarization-diff visualizes the results:

```{python}
#| label: fig-zdt1-scalarization-diff   
...
```{python}
#| label: fig-mo-desirability-function-plot
#| fig-cap: Desirability functions of the mo_conv2_max function
