...
### Method: `rm_initial_design_NA_values(X0,y0)`
...
**What happens in `rm_initial_design_NA_values()`:**
...
`fit_scheduler()` then calls `fit_surrogate()` with the selected data.

### Method: `fit_surrogate()`

Fit a surrogate model (Gaussian Process) to current data.

**What happens in `fit_surrogate()`:**

1. If `max_surrogate_points` set and exceeded: Select subset of points using the `fit_selection_dispatcher()` method.
...
### Method: `fit_selection_dispatcher()`

**What happens in `fit_selection_dispatcher()`:**
...
### Method: `rm_initial_design_NA_values()`
...
The `optimize()` method orchestrates the full optimization process with restart support.
It delegates to `execute_optimization_run()`, which dispatches to `optimize_sequential_run()` (for `n_jobs=1`) or `optimize_steady_state()` (for `n_jobs > 1`).

### Preparation Phase (inside `optimize_sequential_run()` via `_initialize_run()`)
1. `set_seed()` - Set random seed for reproducibility
2. `get_initial_design()` - Generate/process initial sample points
3. `curate_initial_design()` - Remove duplicates, handle repeats
4. `evaluate_function()` - Evaluate objective function
5. `rm_initial_design_NA_values()` - Remove NaN/inf from initial design
6. `check_size_initial_design()` - Validate sufficient points
7. `init_storage()` - Initialize storage (X_, y_, n_iter_)
8. `update_stats()` - Compute mean/variance for noisy functions
9. `_init_tensorboard()` - Log initial design to TensorBoard (if enabled)
10. `get_best_xy_initial_design()` - Identify initial best

### Sequential Optimization Loop (each iteration inside `_run_sequential_loop()`)
11. `fit_scheduler()` - Select data and fit surrogate based on noise handling
    - Internally calls `transform_X()` - Transform to internal scale
    - Internally calls `fit_surrogate()` - Fit Gaussian Process to data
    - Internally calls `fit_selection_dispatcher()` - Subset selection for large datasets
12. `apply_ocba()` - OCBA allocation (if enabled)
13. `suggest_next_infill_point()` - Optimize acquisition function
    - Internally calls `_acquisition_function()`
    - Internally calls `_predict_with_uncertainty()`
14. `update_repeats_infill_points()` - Repeat suggested point for noisy functions
15. `evaluate_function()` - Evaluate at suggested point(s)
16. `_handle_NA_new_points()` - Handle failures with penalties
    - Internally calls `apply_penalty_NA()`
    - Internally calls `remove_nan()`
17. `update_success_rate()` - Update success tracking
18. Check for restart (if `success_rate == 0` for `restart_after_n` consecutive iterations)
19. `update_storage()` - Append new evaluations to storage
    - Internally calls `inverse_transform_X()` - Convert back to original scale
20. `update_stats()` - Update mean/variance statistics
21. `_write_tensorboard_hparams()` - Log hyperparameters (if enabled)
22. `_write_tensorboard_scalars()` - Log scalar metrics (if enabled)
23. `_update_best_main_loop()` - Update best solution

### Finalization
24. `to_all_dim()` - Expand to full dimensions (if dimensionality reduction used)
25. `determine_termination()` - Determine termination reason
26. `_close_tensorboard_writer()` - Close logging (if enabled)
27. `map_to_factor_values()` - Convert factors back to strings
28. Return `OptimizeResult` object
...
- `fit_selection_dispatcher()` - Subset selection for large datasets
- `fit_select_distant_points()` - K-means based distant point selection
- `fit_select_best_cluster()` - Cluster-based best point selection
...
┌─────────────────────────────────────────────────────────────────┐
│                     SPOTOPTIM WORKFLOW                          │
│  optimize() → execute_optimization_run()                       │
│    → optimize_sequential_run() → _run_sequential_loop()        │
└─────────────────────────────────────────────────────────────────┘

INITIALIZATION PHASE (optimize_sequential_run via _initialize_run)
  │
  ├─► set_seed()
  │     └─► Set random seed for reproducibility
  │
...
SEQUENTIAL OPTIMIZATION LOOP (_run_sequential_loop, until max_iter or max_time)
  │
  ├─► fit_scheduler()
  │     ├─► transform_X() - Transform to internal scale
  │     └─► fit_surrogate() - Fit GP to current data
  │           └─► fit_selection_dispatcher() - Subset selection if needed
  │
  ...
  │
  ├─► [Check for restart if success_rate == 0]
  │     └─► Return "RESTART" if threshold reached
  │
  ├─► update_storage()
